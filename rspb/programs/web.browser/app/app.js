      ...
      return {
        ...
        other: function(){
          ...
          if ((ref$ = schema.autoform) != null && ref$.options) {
            ...
            return m('.box', opts.scope !== name ? m('div', m('h5.subtitle', label), m('a.button.is-success', attr.arrLen(name, 'inc'), '+ Add'), m('a.button.is-warning', attr.arrLen(name, 'dec'), '- Rem'), (that = state.arrLen[name]) ? m('.button', that) : void 8) : void 8, (function(){
              var i$, to$, results$ = [];
...
this.modules = [
  {
    name: 'regis',
    full: 'Pendaftaran',
    access: ['regis', 'panduan'],
    icon: 'users',
    color: 'orange'
  }, {
    name: 'bayar',
    full: 'Pembayaran',
    access: ['bayar', 'panduan'],
    icon: 'cash-register',
    color: 'green'
  }, {
    name: 'jalan',
    full: 'Rawat Jalan',
    access: ['jalan', 'farmasi', 'amprah', 'panduan'],
    icon: 'directions',
    color: 'blue'
  }, {
    name: 'inap',
    full: 'Rawat Inap',
    access: ['inap', 'farmasi', 'amprah', 'panduan'],
    icon: 'procedures',
    color: 'red'
  }, {
    name: 'obat',
    full: 'Apotek',
    access: ['obat', 'farmasi', 'amprah', 'panduan'],
    icon: 'mortar-pestle',
    color: 'light-green'
  }, {
    name: 'farmasi',
    full: 'Gudang Farmasi',
    access: ['farmasi', 'amprah', 'panduan'],
    icon: 'book-medical',
    color: 'orange'
  }, {
    name: 'manajemen',
    full: 'Manajemen',
    access: ['manajemen', 'panduan'],
    icon: 'tasks',
    color: 'orange'
  }, {
    name: 'amprah',
    ...
    name: 'depook',
    full: 'Depo OK',
    access: ['depook', 'farmasi', 'amprah', 'panduan'],
    icon: 'mortar-pestle',
    color: 'green'
  }, {
    name: 'radio',
    full: 'Radiologi',
    access: ['radio', 'amprah', 'panduan'],
    icon: 'users',
    color: 'green'
  }, {
    name: 'labor',
    full: 'Laboratorium',
    access: ['labor', 'amprah', 'panduan'],
    icon: 'users',
    color: 'green'
  }, {
    name: 'gizi',
    full: 'Gizi',
    access: ['amprah', 'panduan'],
    icon: 'users',
    color: 'green'
  }, {
    name: 'cssd',
...
this.rupiah = function(it){
  return "Rp " + numeral(+it || 0).format('0,000.00') + ",-";
};
...
if (Meteor.isClient) {
  ...
  this.makePdf = {
    ...
    payRawat: function(arg$){
      var pasien, rawat, rows, items, table, pdf, arr, ref$, ref1$;
      pasien = arg$.pasien, rawat = arg$.rawat, rows = arg$.rows;
  ...
  schema.addRole = {
    ...
    inap: {
      ...
      autoform: {
        type: 'select',
        options: function(){
          return selects.inap.map(function(it){
            return {
              label: it.label,
              value: _.snakeCase(it.label)
            };
  ...
  schema.amprah = function(type){
    return {
      ...
      ruangan: {
        ...
        autoValue: function(){
          var ref$;
          if ((ref$ = userGroup()) === 'jalan' || ref$ === 'inap') {
            return userRole();
          } else {
            return userGroup();
          }
...
if (Meteor.isClient) {
  ...
  comp = {
                        ...
                        if (that = doc.pindah) {
                          coll.pasien.update({
                            ...
                            $push: {
                              rawat: {
                                klinik: that,
                                billRegis: doc.cara_bayar === 1 ? false : true,
        ...
        view: function(){
          ...
          if (attr.pageAccess(['obat', 'depook'])) {
            return m('.content', {
              ...
              ? 'Apotik'
              : userGroup('depook') ? 'Depo OK' : void 8), m('button.button.is-success', {
              onclick: function(){
    ...
    farmasi: function(){
      return {
        view: function(){
          var ref$, jumlah, that, ref1$, ref2$, ref3$, ref4$, ref5$, ref6$, ref7$, ref8$, this$ = this;
          if (attr.pageAccess(['jalan', 'inap', 'obat', 'farmasi', 'depook'])) {
            return m('.content', {
              ...
              ? m('div', (ref$ = userGroup()) === 'obat' || ref$ === 'farmasi' || ref$ === 'depook' ? (jumlah = function(it){
                return it.length;
              }(coll.gudang.find().fetch().filter(function(it){
                var ref$, ref1$, ref2$;
                if (userGroup('obat')) {
                  return !((ref$ = it.treshold) != null && ref$.apotik);
                } else if (userGroup('depook')) {
                  return !((ref1$ = it.treshold) != null && ref1$.depook);
                } else if (userGroup('farmasi')) {
                  return !((ref2$ = it.treshold) != null && ref2$.gudang);
                }
                ...
                return m('tr', {
                  'class': function(){
                    var ref$, ref1$, ref2$;
                    if (userGroup('obat')) {
                      return ((ref$ = i.treshold) != null ? ref$.apotik : void 8) > _.sumBy(i.batch, 'diapotik');
                    } else if (userGroup('depook')) {
                      return ((ref1$ = i.treshold) != null ? ref1$.depook : void 8) > _.sumBy(i.batch, 'didepook');
                    } else if (userGroup('farmasi')) {
                      return ((ref2$ = i.treshold) != null ? ref2$.gudang : void 8) > _.sumBy(i.batch, 'digudang');
                    }
                  }() ? 'has-text-danger' : void 8,
                  ondblclick: function(){
                ...
                onchange: function(e){
                  return Papa.parse(e.target.files[0], {
                    ...
                    step: function(result){
                      ...
                      if (data.password) {
                        return Meteor.call('newUser', data);
                      }
                    }
                  });
                }
...
if (Meteor.isServer) {
  ...
  Meteor.methods({
    ...
    addRole: function(doc){
      var id, roles, group, poli, inap;
      id = doc.id, roles = doc.roles, group = doc.group, poli = doc.poli, inap = doc.inap;
      if (secureMethods({
        name: 'addRole',
        userId: this.userId,
        args: doc
      })) {
        return Roles.addUsersToRoles(id, inap || poli || roles, group);
      }
    },
    rmRole: function(doc){
      if (secureMethods({
    ...
    incomes: function(arg$){
      ...
      if (start < end) {
        ...
        b = a.map(function(i){
          ...
          return _.assign(j, {
            Total: _.sum(['Kartu', 'Karcis', 'Tindakan', 'Obat'].map(function(it){
              return j[it];
            }))
          });
        });
