...
if (Meteor.isClient) {
  this.modForm = function(doc, idbayar){
    var i$, ref$, len$, i, find, begin, stop;
    if (currentRoute() === 'jalan') {
      ...
      _.map(['tindakan', 'labor', 'radio'], function(i){
        var i$, ref$, len$, j, find, results$ = [];
        if (doc[i]) {
          for (i$ = 0, len$ = (ref$ = doc[i]).length; i$ < len$; ++i$) {
            j = ref$[i$];
            j["id" + i] = randomId();
      ...
      for (i$ = 0, len$ = (ref$ = doc.obat != null).length; i$ < len$; ++i$) {
        i = ref$[i$];
        i.idobat = randomId();
        find = _.find(coll.gudang.find().fetch(), fn$);
        i.harga = 0;
...
if (Meteor.isClient) {
  ...
  Template.pasien.helpers({
    ...
    pasiens: function(){
      ...
      } else if ((ref$ = currentRoute()) === 'labor' || ref$ === 'radio' || ref$ === 'obat') {
        elem = {
          'status_bayar': true
        };
        elem[currentRoute()] = {
