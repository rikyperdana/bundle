  ...
  this.autoForm = function(opts){
    ...
    attr = {
      form: {
        ...
        onsubmit: function(e){
          var temp, formFields, formValues, obj, context, after, formTypes, that, ref$;
          e.preventDefault();
...
selects.dokter = function(){
  if (Meteor.isClient) {
    return Meteor.users.find().fetch().filter(function(it){
      var ref$;
      return (ref$ = _.split(it.username, '.')[0]) === 'dr' || ref$ === 'drg';
    }).map(function(it){
      return {
        value: it._id,
        ...
        return arr = [
          {
            ...
            head: 'Diagnosa',
            cell: doc != null ? doc.diagnosa : void 8
          }, {
            head: 'Planning',
      ...
      return {
        view: function(){
          ...
          return m('div', !Meteor.userId() ? m.route.set('/login') : void 8, m('link', {
            ...
            return (it != null ? it.full : void 8) || 'RSPB';
            ...
            var arr, ref$;
            arr = [
              [JSON.stringify((ref$ = Meteor.user()) != null ? ref$.roles : void 8)], [
                'Login', function(){
                  return m.route.set('/login');
                }
              ], [
                'Logout', function(){
                  return [Meteor.logout(), m.route.set('/login'), m.redraw()];
                }
              ]
            ];
            ...
            }, m('span', _.startCase(i.full)), 'regis' === currentRoute() ? m('ul', [['lama', 'Cari Pasien'], ['baru', 'Pasien Baru']].map(function(i){
              return m('li', m('a', {
                href: "/regis/" + i[0],
    ...
    pasien: function(){
      return {
        view: function(){
          ...
          return m('.content', (ref$ = m.route.param('jenis')) === 'baru' || ref$ === 'edit' ? m(autoForm({
            ...
            : (ref$ = m.route.get()) === '/regis/lama' || ref$ === '/jalan'
              ...
              : m.route.param('idpasien')
                ? m('div', {
                ...
                })), currentRoute() === 'regis' ? m('div', m('.button.is-info', {
                  onclick: function(){
                    return makePdf.card(m.route.param('idpasien'));
                  }
                }, m('span', 'Kartu')), m('.button.is-info', {
                  onclick: function(){
                    return makePdf.consent();
                  }
                }, m('span', 'Consent')), m('.button.is-warning', {
                  onclick: function(){
                    return m.route.set("/regis/edit/" + m.route.param('idpasien'));
                  }
                }, m('span', 'Edit')), m('.button.is-success', attr.pasien.showForm.rawat, '+Rawat')) : void 8, state.showAddRawat && m(autoForm({
                  collection: coll.pasien,
                  ...
                  return m('th', _.startCase(i));
                }), userRole('admin') ? m('th', 'Hapus') : void 8)), m('tbody', (ref$ = attr.pasien.poliFilter(that != null ? (ref1$ = that.rawat) != null ? ref1$.reverse() : void 8 : void 8)) != null ? ref$.map(function(i){
                  var that;
                  return m('tr', [hari(i.tanggal), look('klinik', i.klinik).label, look('cara_bayar', i.cara_bayar).label, (that = i.dokter) ? _.startCase(Meteor.users.find(that).username) : void 8].concat(
                    slice$.call(['billRegis', 'status_bayar'].map(function(it){
                      ...
                      }, m('span', 'Cek')) : void 8, userRole('admin') ? m('.button.is-danger', {
                        ondblclick: function(){
    ...
    bayar: function(){
      return {
        view: function(){
          ...
          }) : void 8, uraian = [!((ref1$ = coll.pasien.findOne(state.modal.pasienId).rawat) != null && ((ref2$ = ref1$[0]) != null && ref2$.billRegis)) ? ['Cetak Kartu', 10000] : void 8, !state.modal.billRegis ? ['Daftar Rawat', 30000] : void 8].concat(slice$.call(tindakans || [])), params = ['pasienId', 'idrawat'].map(function(it){
    ...
    farmasi: function(){
      return {
        view: function(){
          ...
          return m('.content', !m.route.param('idbarang')
            ? m('div', m('form', {
              onsubmit: function(e){
                e.preventDefault();
                if (e.target[0].value.length > 3) {
            ...
            }, m('span', '+Tambah Jenis Obat')) : void 8, state.showForm ? (m('h5', 'Form Barang Farmasi'), m(autoForm({
            ...
            }))) : void 8, m('table.table', m('thead', m('tr', attr.gudang.headers.farmasi.map(function(i){
              return m('th', _.startCase(i));
            }))), m('tbody', coll.gudang.find().fetch().map(function(i){
              return m('tr', {
                ondblclick: function(){
                  return m.route.set("/farmasi/" + i._id);
                }
              }, m('td', look('barang', i.jenis).label), m('td', i.nama), ['digudang', 'diapotik'].map(function(j){
                return m('td', _.sumBy(i.batch, j));
              }), userRole() === 'admin' && m('td', m('.button.is-danger', {
                onclick: function(){
                  return state.modal = i;
                }
              }, m('span', 'Hapus'))));
            ...
            }) : void 8), (ref1$ = roles()) != null && ref1$.farmasi ? m('.button.is-warning', {
              onclick: function(){
                return state.showForm = !state.showForm;
              }
    ...
    amprah: function(){
      return {
        view: function(){
          ...
          return m('.content', m('br', {
          ...
          }), m('.columns', _.compact(attr.amprah.reqForm()).map(function(type){
            return m('.column', m('.button.is-primary', {
              onclick: function(){
                return state.showForm = !state.showForm;
              }
            }, m('span', "Request " + _.upperCase(type))), m('br'), state.showForm && !userGroup('farmasi') ? (m('h5', 'Form Amprah'), m(autoForm({
            ...
            return m('tr', tds(arr = [
              hari(i.tanggal_minta), _.startCase(i.ruangan), _.startCase(function(it){
                return it.username;
            ...
            title: 'Respon Amprah',
            content: m('div', m('table.table', m('thead', m('tr', ['nama', 'diminta', 'sedia'].map(function(i){
              return m('th', _.startCase(i));
            }))), m('tbody', m('tr', tds(arr = [
              (ref$ = look2('gudang', that.nama)) != null ? ref$.nama : void 8, that.jumlah, _.sum(look2('gudang', that.nama).batch.map(function(i){
                if (userGroup('obat')) {
                  return i.digudang;
                } else {
                  return i.diapotik;
...
if (Meteor.isServer) {
  ...
  Meteor.methods({
    ...
    serahAmprah: function(doc){
      ...
      return coll.gudang.update(doc.nama, {
        $set: {
